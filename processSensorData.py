...
data_dir = 'data/Nov26_squaremotion/'

if __name__ == '__main__':
    orientation_signal = np.genfromtxt(data_dir + 'Orientation.csv', delimiter=',')
    acceleration_signal = np.genfromtxt(data_dir + 'Accelerometer.csv', delimiter=',')
    ...
    ax.scatter3D(xx[:, 0], xx[:, 1], xx[:, 2], c=xx[:, 2], cmap='Greens');
    ax.set_xlabel('X')
    ax.set_ylabel('Y')
    ax.set_zlabel('Z')
    # fig, ax = plt.subplots(1, 1, figsize=(18, 18))
    # ax.plot(tt, xx[:, 0], label="x")
    # ax.plot(tt, xx[:, 1], label="y")
    # ax.plot(tt, xx[:, 2], label="z")
    # ax.legend()
    plt.show()
